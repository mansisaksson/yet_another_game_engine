...
{
	transform.location += vector3(m_movement_input.y * delta_time, m_movement_input.x * delta_time, 0) * m_movement_speed;
}
...
{
	const auto ship_forward = transform.rotation.get_forward();
	get_scene()->spawn_entity<bullet_entity>(transform, ship_forward * 3.f, 5.f);
}
