...
class tavla_content : public tavla
{
public:
	...
	private:
		std::weak_ptr<tavla_content> m_parent;
		std::shared_ptr<tavla_content> m_content;
	...
	public:
		slot(std::shared_ptr<tavla_content> t_parent)
			: m_parent(t_parent)
			, width(100)
			, height(100)
		{}
...
protected:
	std::vector<std::shared_ptr<tavla_content::slot>> m_child_slots;
...
public:
	...
	inline const std::vector<std::shared_ptr<tavla_content::slot>>& get_child_slots() const { return m_child_slots; }

	static void tick_tavla_tree(const std::shared_ptr<tavla>& t_root_tavla, const float t_delta_time)
	{
		struct local
		{
			static void tick_slots_recursive(const std::shared_ptr<tavla_content::slot>& t_slot, const float t_delta_time)
			{
